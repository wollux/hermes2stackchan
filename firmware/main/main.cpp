...
constexpr int64_t kDisplayTouchHoldToRecordMs = 260;
constexpr int64_t kDisplayTouchTapMaxMs = 240;
#if CONFIG_FREERTOS_UNICORE
constexpr BaseType_t kTtsAudioCore = tskNO_AFFINITY;
constexpr BaseType_t kTtsFaceCore = tskNO_AFFINITY;
#else
constexpr BaseType_t kTtsAudioCore = 1;
constexpr BaseType_t kTtsFaceCore = 0;
#endif
...
char g_pre_tts_face_emotion[24] = "neutral";
int g_pre_tts_face_intensity_pct = 60;
bool g_pre_recording_info_mode = false;
bool g_pre_tts_info_mode = false;
bool g_info_mode_active = false;
char g_info_time[16] = "";
char g_info_date[24] = "";
char g_info_weekday[24] = "";
...
volatile int g_motion_gaze_render_dx = 0;
volatile int g_motion_gaze_render_dy = 0;
volatile bool g_motion_gaze_active = false;
volatile uint32_t g_motion_gaze_revision = 0;
bool g_force_face_redraw = false;
bool g_face_partial_update_next = false;
...
volatile bool g_tts_stop_requested = false;
volatile int g_tts_playback_level_pct = 0;
volatile uint32_t g_tts_playback_level_revision = 0;
...
void flush_frame_region(int x, int y, int w, int h)
{
    if (!g_panel || !g_framebuffer || w <= 0 || h <= 0) {
        return;
    }
    x = clamp_int(x, 0, kWidth - 1);
    y = clamp_int(y, 0, kHeight - 1);
    ...
    if (!g_lcd_dma_buffer || g_lcd_dma_lines <= 0) {
        static uint16_t row[kWidth];
        for (int yy = 0; yy < h; ++yy) {
            std::memcpy(row, g_framebuffer + (y + yy) * kWidth + x, w * sizeof(uint16_t));
    ...
    for (int yy = 0; yy < h; yy += lines_per_chunk) {
        ...
        for (int line = 0; line < chunk_lines; ++line) {
            std::memcpy(g_lcd_dma_buffer + line * w,
                        g_framebuffer + (y + yy + line) * kWidth + x,
                        w * sizeof(uint16_t));
        }
        if (g_lcd_transfer_done) {
            while (xSemaphoreTake(g_lcd_transfer_done, 0) == pdTRUE) {
        ...
        ESP_ERROR_CHECK_WITHOUT_ABORT(err);
        if (err == ESP_OK && g_lcd_transfer_done &&
            xSemaphoreTake(g_lcd_transfer_done, pdMS_TO_TICKS(80)) != pdTRUE) {
            ESP_LOGD(kTag, "LCD partial transfer timeout");
        }
    }
}
...
void draw_info_screen(const char* time_text, const char* date_text, const char* weekday_text);
...
struct FaceFrameGuard {
    ...
    explicit FaceFrameGuard(bool partial_update = false)
        : active(begin_frame()), partial(partial_update) {}
    ...
    ~FaceFrameGuard()
    {
        if (active) {
            ...
            if (partial) {
                g_framebuffer_active = false;
                flush_frame_region(0, 18, kWidth, 204);
                if (g_display_mutex) {
                    xSemaphoreGive(g_display_mutex);
                }
            } else {
...
void draw_audio_wave_mouth(int cx, int cy, int width, int level_pct, uint16_t color)
{
    const int half = std::max(18, width / 2);
    const int level = clamp_int(level_pct, 0, 100);
    const int amp = clamp_int(3 + level / 7, 3, 17);
    const int phase = static_cast<int>((esp_timer_get_time() / 70000) % 4);
    constexpr int kSegments = 8;

    int prev_x = cx - half;
    int prev_y = cy + 2;
    for (int i = 1; i <= kSegments; ++i) {
        const int x = cx - half + (width * i) / kSegments;
        const int center_distance = std::abs(i - (kSegments / 2));
        const int envelope = clamp_int(100 - center_distance * 18, 32, 100);
        const int sign = ((i + phase) % 2 == 0) ? -1 : 1;
        const int smile_lift = 2 + std::abs(i - (kSegments / 2)) / 2;
        const int y = cy + smile_lift + sign * amp * envelope / 100;
        draw_line(prev_x, prev_y, x, y, color, 4);
        prev_x = x;
        prev_y = y;
    }
}
...
void draw_face_extras()
{
    // The robot-face template is the single display truth: no extra overlays on the face.
}
...
bool is_robot_template_emotion(const char* emotion)
{
    return str_eq(emotion, "neutral") ||
           str_eq(emotion, "friendly") ||
           str_eq(emotion, "super_happy") ||
           str_eq(emotion, "happy") ||
           str_eq(emotion, "thankful") ||
           str_eq(emotion, "love") ||
           str_eq(emotion, "sad") ||
           str_eq(emotion, "angry") ||
           str_eq(emotion, "surprised") ||
           str_eq(emotion, "tired") ||
           str_eq(emotion, "annoyed") ||
           str_eq(emotion, "confused") ||
           str_eq(emotion, "scared") ||
           str_eq(emotion, "calm") ||
           str_eq(emotion, "curious") ||
           str_eq(emotion, "playful") ||
           str_eq(emotion, "focused") ||
           str_eq(emotion, "concerned") ||
           str_eq(emotion, "thinking") ||
           str_eq(emotion, "listening") ||
           str_eq(emotion, "speaking") ||
           str_eq(emotion, "charging");
}

bool accepts_robot_template_face_command(const char* emotion)
{
    if (!emotion || !*emotion) {
        return true;
    }
    if (is_robot_template_emotion(emotion)) {
        return true;
    }

    return is_transient_face_emotion(emotion) ||
           str_eq(emotion, "question") ||
           str_eq(emotion, "curious") ||
           str_eq(emotion, "thinking") ||
           str_eq(emotion, "surprise") ||
           str_eq(emotion, "sleep") ||
           str_eq(emotion, "sleepy") ||
           str_eq(emotion, "bored") ||
           str_eq(emotion, "grumble") ||
           str_eq(emotion, "skeptical") ||
           str_eq(emotion, "offended") ||
           str_eq(emotion, "friendly") ||
           str_eq(emotion, "super_happy") ||
           str_eq(emotion, "thankful") ||
           str_eq(emotion, "charging") ||
           str_eq(emotion, "panic") ||
           str_eq(emotion, "help") ||
           str_eq(emotion, "face_down") ||
           str_eq(emotion, "error") ||
           str_eq(emotion, "battery_low") ||
           str_eq(emotion, "calm") ||
           str_eq(emotion, "playful") ||
           str_eq(emotion, "focused") ||
           str_eq(emotion, "concerned") ||
...
const char* normalize_face_emotion(const char* emotion)
{
    ...
    if (str_eq(emotion, "dead") || str_eq(emotion, "glitch")) {
        return "tired";
    }
    ...
    if (str_eq(emotion, "calm")) {
        return "calm";
    }
    if (str_eq(emotion, "playful")) {
        return "playful";
    }
    if (str_eq(emotion, "focused")) {
        return "focused";
    }
    if (str_eq(emotion, "concerned")) {
        return "concerned";
    }
    if (str_eq(emotion, "listening")) {
        return "listening";
    }
    if (str_eq(emotion, "speaking")) {
        return "speaking";
    }
    if (str_eq(emotion, "question") ||
        str_eq(emotion, "curious") ||
        str_eq(emotion, "thinking")) {
        return str_eq(emotion, "curious") ? "curious" : "confused";
    }
    ...
    if (str_eq(emotion, "sleep") ||
        str_eq(emotion, "sleepy") ||
        str_eq(emotion, "bored") ||
        str_eq(emotion, "micro_sleep") ||
        str_eq(emotion, "yawn")) {
        return "tired";
    }
    if (str_eq(emotion, "grumble") ||
        str_eq(emotion, "grumble_mouth") ||
        str_eq(emotion, "skeptical") ||
        str_eq(emotion, "offended")) {
        return "annoyed";
    }
...
bool is_transient_face_emotion(const char* emotion)
{
    return str_eq(emotion, "blink") ||
           str_eq(emotion, "soft_blink") ||
           str_eq(emotion, "breathe") ||
           str_eq(emotion, "deep_breathe") ||
           str_eq(emotion, "glance_left") ||
           str_eq(emotion, "glance_right") ||
           str_eq(emotion, "glance_up") ||
           str_eq(emotion, "glance_down") ||
           str_eq(emotion, "look_left") ||
           str_eq(emotion, "look_right") ||
           str_eq(emotion, "look_up") ||
           str_eq(emotion, "look_down") ||
           str_eq(emotion, "brow_raise") ||
           str_eq(emotion, "brow_soft") ||
           str_eq(emotion, "brow_skeptic") ||
           str_eq(emotion, "brow_skeptic_left") ||
           str_eq(emotion, "brow_skeptic_right") ||
           str_eq(emotion, "brow_wiggle") ||
           str_eq(emotion, "mouth_smile") ||
           str_eq(emotion, "mouth_tiny") ||
           str_eq(emotion, "mouth_wiggle");
}
...
void draw_simple_mouth(int cx, int y, int width, int height, int mode, uint16_t color)
{
	...
	if (mode == 1) {
		...
		draw_audio_wave_mouth(cx, y, width, static_cast<int>(g_tts_playback_level_pct), color);
	} else {
...
bool draw_speaking_mouth_partial(int level_pct)
{
    if (!g_framebuffer || !g_panel || g_display_sleeping || std::strcmp(g_ui_mode, "speaking") != 0) {
        return false;
    }
    if (g_display_mutex && xSemaphoreTake(g_display_mutex, pdMS_TO_TICKS(12)) != pdTRUE) {
        return false;
    }

    constexpr int face_x = 48;
    constexpr int face_y = 24;
    constexpr int face_w = 224;
    constexpr int face_h = 180;
    const int level = clamp_int(level_pct, 0, 100);
    const int64_t now_us = esp_timer_get_time();
    const float slow = std::sin(static_cast<float>(now_us) / 430000.0f);
    const float glance = std::sin(static_cast<float>(now_us) / 690000.0f);
    const uint16_t white = rgb565(245, 250, 255);

    int pupil_dx = static_cast<int>(std::lround(glance * 4.0f));
    int pupil_dy = static_cast<int>(std::lround(slow * 2.0f)) - 1;
    const bool motion_gaze_visible =
        g_motion_gaze_active ||
        g_motion_gaze_render_dx != 0 ||
        g_motion_gaze_render_dy != 0;
    if (motion_gaze_visible) {
        pupil_dx = clamp_int(static_cast<int>(g_motion_gaze_render_dx), -14, 14);
        pupil_dy = clamp_int(static_cast<int>(g_motion_gaze_render_dy), -10, 10);
    }

    const int eye_open = motion_gaze_visible
        ? 32
        : clamp_int(31 + level / 32 + static_cast<int>(std::lround(slow * 2.0f)), 25, 38);
    const int brow_lift = 0;

    g_framebuffer_active = true;
    draw_rect(face_x, face_y, face_w, face_h, kBlack);
    draw_single_eye_scaled_pupil(100, 84, 20, eye_open, pupil_dx, pupil_dy, 88, white);
    draw_single_eye_scaled_pupil(220, 84, 20, eye_open, pupil_dx, pupil_dy, 88, white);
    draw_line(70, 34 + brow_lift, 130, 30 + brow_lift, white, 5);
    draw_line(190, 30 + brow_lift, 250, 34 + brow_lift, white, 5);
    draw_audio_wave_mouth(160, 174, 82 + level / 5, level, white);
    g_framebuffer_active = false;

    flush_frame_region(face_x, face_y, face_w, face_h);
...
{
    wake_display_if_needed();
    const bool partial_face_update = std::strcmp(g_ui_mode, "face") == 0 && !g_info_mode_active;
    copy_ui_mode(g_face_extra_mode == FaceExtraMode::VoiceWaveform ? "recording" : "face");
    FaceFrameGuard frame(partial_face_update && std::strcmp(g_ui_mode, "face") == 0);
                                 ...
                                 int left_tilt, int right_tilt, int intensity_pct)
{
	...
	draw_simple_mouth(160 + mouth_dx, mouth_y + mouth_dy, 64 + pulse * 3, 18 + pulse, mouth_mode, white);
}
...
void draw_face_fx(int blush_alpha_pct, bool sweat, bool anger, int hearts_alpha_pct, bool dead, bool glitch, uint16_t accent)
{
    ...
    if (blush_alpha > 0) {
        ...
        const uint16_t blush_color = rgb565_scaled(245, 250, 255, blush_alpha / 4);
        draw_ellipse(58, 142 + lift, rx, ry, blush_color);
        draw_ellipse(262, 142 + lift, rx, ry, blush_color);
    }
    if (sweat) {
        draw_ellipse(260, 48, 7, 11, accent);
        draw_line(257, 38, 264, 28, accent, 4);
    }
    if (anger) {
        draw_line(248, 26, 270, 16, accent, 5);
        draw_line(260, 14, 260, 38, accent, 5);
        draw_line(244, 24, 276, 34, accent, 5);
    }
    ...
    if (hearts_alpha > 0) {
        ...
        const uint16_t heart_color = rgb565_scaled(245, 250, 255, hearts_alpha);
    ...
    if (glitch) {
        ...
        draw_rect(226, 118, 58, 4, accent);
        draw_rect(112, 188, 78, 3, accent);
    }
}
...
bool build_face_pose(const char* emotion, int intensity_pct, FacePose& pose, bool include_motion_gaze = true)
{
    ...
    if (str_eq(emotion, "soft_blink")) {
    ...
    } else if (str_eq(emotion, "neutral") || str_eq(emotion, "calm")) {
        pose.draw_brows = false;
        pose.mouth_mode = 1;
        pose.mouth_width = str_eq(emotion, "calm") ? 70 : 76;
        pose.mouth_height = str_eq(emotion, "calm") ? 20 : 23;
    } else if (str_eq(emotion, "playful")) {
        pose.left_ry = pose.right_ry = 34 + pulse / 2;
        pose.left_pupil_dx = pose.right_pupil_dx = 2;
        pose.mouth_mode = 1;
        pose.mouth_width = 86;
        pose.mouth_height = 28 + pulse;
        pose.lb[1] = 32; pose.lb[3] = 26; pose.rb[1] = 26; pose.rb[3] = 32;
    } else if (str_eq(emotion, "curious")) {
        pose.left_pupil_dx = -4;
        pose.right_pupil_dx = 4;
        pose.left_pupil_dy = pose.right_pupil_dy = -5;
        pose.mouth_mode = 7;
        pose.mouth_width = 58;
        pose.mouth_height = 16;
        pose.lb[1] = 23; pose.lb[3] = 43; pose.rb[1] = 42; pose.rb[3] = 27;
    } else if (str_eq(emotion, "focused")) {
        pose.left_ry = pose.right_ry = 24 + pulse / 2;
        pose.pupil_scale = 82;
        pose.mouth_mode = 4;
        pose.mouth_width = 62;
        pose.mouth_height = 9;
        pose.lb[1] = 38; pose.lb[3] = 34; pose.rb[1] = 34; pose.rb[3] = 38;
    } else if (str_eq(emotion, "concerned")) {
        pose.left_ry = pose.right_ry = 28 + pulse / 2;
        pose.left_pupil_dy = pose.right_pupil_dy = 5;
        pose.mouth_mode = 8;
        pose.mouth_width = 60;
        pose.mouth_height = 16;
        pose.lb[1] = 30; pose.lb[3] = 48; pose.rb[1] = 48; pose.rb[3] = 30;
        ...
        pose.left_ry = pose.right_ry = 36 + pulse;
        pose.pupil_scale = 92;
        pose.left_pupil_dy = pose.right_pupil_dy = -2;
        ...
        pose.left_ry = pose.right_ry = str_eq(emotion, "thankful") ? 32 + pulse : 35 + pulse;
        pose.pupil_scale = 92;
        pose.left_pupil_dy = pose.right_pupil_dy = -1;
        ...
        const int voice = clamp_int(static_cast<int>(g_voice_level_pct), 0, 100);
        pose.left_rx = pose.right_rx = 20 + pulse / 2;
        pose.left_ry = pose.right_ry = 34 + voice / 28;
        pose.left_pupil_dy = pose.right_pupil_dy = -2;
        pose.pupil_scale = 84;
        pose.mouth_mode = 1; pose.mouth_width = 56 + voice / 7; pose.mouth_height = 14 + voice / 16;
        pose.lb[1] = 32; pose.lb[3] = 25; pose.rb[1] = 25; pose.rb[3] = 32;
    } else if (str_eq(emotion, "speaking")) {
        const int level = clamp_int(static_cast<int>(g_tts_playback_level_pct), 0, 100);
        const int open = clamp_int(18 + level * 2, 18, 92);
        pose.left_ry = pose.right_ry = 32 + level / 28;
        pose.pupil_scale = 88;
        pose.mouth_mode = 9;
        pose.mouth_width = 78 + open / 4;
        pose.mouth_height = 12 + open / 6;
        pose.lb[1] = 34; pose.lb[3] = 30; pose.rb[1] = 30; pose.rb[3] = 34;
    ...
    pose.eye_color = white;
    pose.mouth_color = white;
    pose.brow_color = white;
...
void render_face_pose(const FacePose& pose)
{
    ...
    const int eye_y = 84;
    const int mouth_y = 168;
    FaceFrameGuard frame(g_face_partial_update_next);
    clear(kBlack);

    if (pose.heart_eyes) {
        draw_tiny_heart(left_x, eye_y, 42, pose.eye_color);
        draw_tiny_heart(right_x, eye_y, 42, pose.eye_color);
    } else if (pose.x_eyes) {
...
bool draw_motion_gaze_partial(int pupil_dx, int pupil_dy)
{
    if (!g_framebuffer || !g_panel || g_display_sleeping || std::strcmp(g_ui_mode, "face") != 0) {
        return false;
    }
    if (g_display_mutex && xSemaphoreTake(g_display_mutex, pdMS_TO_TICKS(10)) != pdTRUE) {
        return false;
    }

    FacePose pose = {};
    const char* emotion = normalize_face_emotion(g_face_emotion);
    if (!build_face_pose(emotion, g_face_intensity_pct, pose, false)) {
        build_face_pose("playful", 68, pose, false);
    }
    if (!pose.heart_eyes && !pose.x_eyes) {
        if (!pose.left_line) {
            pose.left_pupil_dx = clamp_int(pupil_dx, -18, 18);
            pose.left_pupil_dy = clamp_int(pupil_dy, -14, 14);
        }
        if (!pose.right_line) {
            pose.right_pupil_dx = clamp_int(pupil_dx, -18, 18);
            pose.right_pupil_dy = clamp_int(pupil_dy, -14, 14);
        }
        pose.pupil_scale = clamp_int(std::min(pose.pupil_scale, 76), 35, 130);
    }
    ...
    constexpr int w = 228;
    constexpr int h = 118;
    constexpr int left_x = 100;
    constexpr int right_x = 220;
    constexpr int eye_y = 84;

    g_framebuffer_active = true;
    draw_rect(x, y, w, h, kBlack);
    if (pose.heart_eyes) {
        draw_tiny_heart(left_x, eye_y, 42, pose.eye_color);
        draw_tiny_heart(right_x, eye_y, 42, pose.eye_color);
    } else if (pose.x_eyes) {
...
bool draw_mood_preset(const char* emotion, int intensity_pct)
{
    ...
    if (str_eq(emotion, "soft_blink")) {
        ...
        left_ry = right_ry = 36 + pulse;
        pupil_scale = 92;
        left_pupil_dy = right_pupil_dy = -2;
        mouth_mode = 1; mouth_width = 98; mouth_height = 34 + pulse; blush = true; hearts = true;
        lb[1] = 26; lb[3] = 20; rb[1] = 20; rb[3] = 26;
    } else if (str_eq(emotion, "happy") || str_eq(emotion, "thankful")) {
        left_ry = right_ry = str_eq(emotion, "thankful") ? 32 + pulse : 35 + pulse;
        pupil_scale = 92;
        left_pupil_dy = right_pupil_dy = -1;
...
bool animate_template_transient_face(const char* emotion, int intensity_pct);

void animate_transient_face(const char* emotion, int intensity_pct)
{
    if (animate_template_transient_face(emotion, intensity_pct)) {
        return;
    }
...
struct RobotFaceConfig {
    int eye_open_pct = 100;
    int eye_size_pct = 100;
    int pupil_pct = 100;
    int brow_l[4] = {77, 71, 122, 71};
    int brow_r[4] = {198, 71, 243, 71};
    int mouth[6] = {122, 173, 160, 173, 198, 173};
    bool mouth_o = false;
    int look_x = 0;
    int look_y = 0;
    uint16_t line_color = 0;
    uint16_t accent_color = 0;
    bool blush = false;
    bool sweat = false;
    bool anger = false;
    bool dead = false;
...
void copy_robot_points(int (&target)[4], int a, int b, int c, int d)
{
    target[0] = a;
    target[1] = b;
    target[2] = c;
...
void copy_robot_mouth(int (&target)[6], int a, int b, int c, int d, int e, int f)
{
    target[0] = a;
    target[1] = b;
    target[2] = c;
...
void nudge_robot_brow_y(int (&target)[4], int start_delta, int end_delta)
{
    target[1] = clamp_int(target[1] + start_delta, 42, 86);
    target[3] = clamp_int(target[3] + end_delta, 42, 86);
}

void apply_robot_brow_pose(RobotFaceConfig& config, int brow_mode, int brow_phase)
{
    const int phase = clamp_int(brow_phase, -4, 4);
    const int amount = std::abs(phase);
    if (brow_mode == 1) {
        nudge_robot_brow_y(config.brow_l, -amount * 3, -amount * 2);
        nudge_robot_brow_y(config.brow_r, -amount * 2, -amount * 3);
    } else if (brow_mode == 2) {
        nudge_robot_brow_y(config.brow_l, -amount * 4, -amount * 2);
        nudge_robot_brow_y(config.brow_r, amount, amount * 2);
    } else if (brow_mode == 3) {
        nudge_robot_brow_y(config.brow_l, amount * 2, amount);
        nudge_robot_brow_y(config.brow_r, -amount * 2, -amount * 4);
    } else if (brow_mode == 4) {
        nudge_robot_brow_y(config.brow_l, -amount * 2, -amount);
        nudge_robot_brow_y(config.brow_r, -amount, -amount * 2);
    } else if (brow_mode == 5) {
        const int direction = phase < 0 ? -1 : 1;
        nudge_robot_brow_y(config.brow_l, -direction * amount * 3, -direction * amount);
        nudge_robot_brow_y(config.brow_r, direction * amount, direction * amount * 3);
    }
}
...
RobotFaceConfig robot_face_config(const char* emotion, int intensity_pct)
{
    ...
    config.eye_open_pct = clamp_int(100 + awake_boost, 82, 118);

    if (str_eq(emotion, "calm") || str_eq(emotion, "neutral")) {
        config.eye_open_pct = 98;
        config.eye_size_pct = 100;
        config.pupil_pct = 100;
        copy_robot_points(config.brow_l, 77, 72, 122, 72);
        copy_robot_points(config.brow_r, 198, 72, 243, 72);
        copy_robot_mouth(config.mouth, 120, 170, 160, 188, 200, 170);
    } else if (str_eq(emotion, "playful") || str_eq(emotion, "happy")) {
        config.eye_open_pct = 92;
        config.eye_size_pct = 103;
        config.pupil_pct = 95;
        copy_robot_points(config.brow_l, 77, 66, 122, 62);
        copy_robot_points(config.brow_r, 198, 62, 243, 66);
        copy_robot_mouth(config.mouth, 115, 164, 160, 195, 205, 164);
    } else if (str_eq(emotion, "curious")) {
        config.eye_open_pct = 102;
        config.eye_size_pct = 101;
        config.pupil_pct = 95;
        copy_robot_points(config.brow_l, 77, 58, 122, 72);
        copy_robot_points(config.brow_r, 198, 76, 243, 62);
        copy_robot_mouth(config.mouth, 124, 176, 151, 164, 180, 176);
        config.look_y = -3;
    } else if (str_eq(emotion, "focused")) {
        config.eye_open_pct = 76;
        config.eye_size_pct = 100;
        config.pupil_pct = 88;
        copy_robot_points(config.brow_l, 77, 70, 122, 67);
        copy_robot_points(config.brow_r, 198, 67, 243, 70);
        copy_robot_mouth(config.mouth, 126, 176, 160, 176, 194, 176);
    } else if (str_eq(emotion, "concerned")) {
        config.eye_open_pct = 92;
        config.pupil_pct = 104;
        copy_robot_points(config.brow_l, 77, 66, 122, 80);
        copy_robot_points(config.brow_r, 198, 80, 243, 66);
        copy_robot_mouth(config.mouth, 125, 184, 160, 164, 195, 184);
        config.look_y = 4;
    } else if (str_eq(emotion, "listening")) {
        config.eye_open_pct = 106;
        config.eye_size_pct = 102;
        config.pupil_pct = 90;
        copy_robot_points(config.brow_l, 77, 65, 122, 59);
        copy_robot_points(config.brow_r, 198, 59, 243, 65);
        copy_robot_mouth(config.mouth, 132, 170, 160, 183, 188, 170);
        config.look_y = -2;
    } else if (str_eq(emotion, "speaking")) {
        const int level = clamp_int(static_cast<int>(g_tts_playback_level_pct), 0, 100);
        config.eye_open_pct = 100;
        config.eye_size_pct = 101;
        config.pupil_pct = 90;
        config.look_y = -1;
        copy_robot_mouth(config.mouth,
                         118 - level / 14,
                         174,
                         160,
                         184 + level / 9,
        ...
        config.eye_open_pct = 84;
        config.eye_size_pct = 98;
        config.pupil_pct = 105;
        copy_robot_points(config.brow_l, 77, 66, 122, 78);
        copy_robot_points(config.brow_r, 198, 78, 243, 66);
        copy_robot_mouth(config.mouth, 122, 186, 160, 161, 198, 186);
        config.look_y = 5;
    } else if (str_eq(emotion, "angry")) {
        config.eye_open_pct = 88;
        config.pupil_pct = 85;
        copy_robot_points(config.brow_l, 76, 62, 123, 78);
        copy_robot_points(config.brow_r, 197, 78, 244, 62);
        copy_robot_mouth(config.mouth, 122, 182, 160, 173, 198, 182);
    } else if (str_eq(emotion, "surprised")) {
        config.eye_open_pct = 118;
        config.eye_size_pct = 108;
        config.pupil_pct = 65;
        copy_robot_points(config.brow_l, 77, 55, 122, 55);
        copy_robot_points(config.brow_r, 198, 55, 243, 55);
        config.mouth_o = true;
    } else if (str_eq(emotion, "tired")) {
        config.eye_open_pct = 58;
        config.eye_size_pct = 98;
        copy_robot_points(config.brow_l, 77, 77, 122, 77);
        copy_robot_points(config.brow_r, 198, 77, 243, 77);
        copy_robot_mouth(config.mouth, 128, 179, 160, 176, 192, 179);
        config.look_x = -3;
        config.look_y = 3;
    } else if (str_eq(emotion, "annoyed")) {
        config.eye_open_pct = 64;
        config.pupil_pct = 90;
        copy_robot_points(config.brow_l, 77, 68, 122, 69);
        copy_robot_points(config.brow_r, 198, 68, 243, 66);
        copy_robot_mouth(config.mouth, 125, 176, 160, 179, 195, 176);
        config.look_x = 5;
    } else if (str_eq(emotion, "confused")) {
        config.eye_open_pct = 96;
        config.pupil_pct = 95;
        copy_robot_points(config.brow_l, 77, 58, 122, 72);
        copy_robot_points(config.brow_r, 198, 76, 243, 62);
        copy_robot_mouth(config.mouth, 124, 176, 151, 164, 180, 176);
        config.look_x = -4;
    } else if (str_eq(emotion, "scared")) {
        config.eye_open_pct = 116;
        config.eye_size_pct = 105;
        config.pupil_pct = 70;
        copy_robot_points(config.brow_l, 77, 56, 122, 62);
        copy_robot_points(config.brow_r, 198, 62, 243, 56);
        copy_robot_mouth(config.mouth, 134, 184, 160, 193, 186, 184);
        config.look_x = -4;
        config.sweat = true;
    } else if (str_eq(emotion, "love")) {
        config.eye_open_pct = 100;
        config.eye_size_pct = 102;
        copy_robot_points(config.brow_l, 77, 59, 122, 59);
        copy_robot_points(config.brow_r, 198, 59, 243, 59);
        copy_robot_mouth(config.mouth, 115, 165, 160, 198, 205, 165);
    } else if (str_eq(emotion, "dead")) {
        config.eye_open_pct = 62;
        config.pupil_pct = 80;
        copy_robot_points(config.brow_l, 77, 77, 122, 77);
        copy_robot_points(config.brow_r, 198, 77, 243, 77);
        copy_robot_mouth(config.mouth, 126, 180, 160, 176, 194, 180);
    } else if (str_eq(emotion, "glitch")) {
        config.eye_open_pct = 100;
        config.eye_size_pct = 102;
        config.pupil_pct = 90;
        copy_robot_points(config.brow_l, 77, 64, 122, 76);
        copy_robot_points(config.brow_r, 198, 76, 243, 64);
        copy_robot_mouth(config.mouth, 122, 178, 160, 168, 198, 178);
        config.look_x = 4;
    }
    return config;
}
...
void draw_robot_face_config(RobotFaceConfig config, bool include_motion_gaze)
{
    ...
    if (include_motion_gaze &&
        (g_motion_gaze_active ||
         g_motion_gaze_render_dx != 0 ||
         g_motion_gaze_render_dy != 0)) {
        look_x = clamp_int(static_cast<int>(g_motion_gaze_render_dx), -18, 18);
        look_y = clamp_int(static_cast<int>(g_motion_gaze_render_dy), -14, 14);
    }
    ...
    const int left_x = 99;
    const int right_x = 221;
    const int eye_y = 109;
    const int eye_rx = clamp_int(27 * config.eye_size_pct / 100, 16, 36);
    const int eye_ry = clamp_int(29 * config.eye_open_pct / 100, 4, 40);
    const int pupil_r = clamp_int(7 * config.pupil_pct / 100, 0, 10);

    if (config.blush) {
        draw_ellipse(86, 154, 14, 5, rgb565(8, 34, 38));
        draw_ellipse(234, 154, 14, 5, rgb565(8, 34, 38));
    }

    draw_line(config.brow_l[0], config.brow_l[1], config.brow_l[2], config.brow_l[3], config.line_color, 5);
    draw_line(config.brow_r[0], config.brow_r[1], config.brow_r[2], config.brow_r[3], config.line_color, 5);

    draw_ellipse(left_x, eye_y, eye_rx, eye_ry, config.line_color);
    draw_ellipse(right_x, eye_y, eye_rx, eye_ry, config.line_color);
    if (pupil_r > 0 && eye_ry > 5) {
        draw_ellipse(left_x + look_x, eye_y + look_y, pupil_r, pupil_r, screen);
        draw_ellipse(right_x + look_x, eye_y + look_y, pupil_r, pupil_r, screen);
    }
    if (config.heart) {
        draw_tiny_heart(68, 78, 13, config.line_color);
        draw_tiny_heart(252, 78, 13, config.line_color);
    }

    if (config.dead) {
        draw_line(left_x - 18, eye_y - 18, left_x + 18, eye_y + 18, screen, 6);
        draw_line(left_x + 18, eye_y - 18, left_x - 18, eye_y + 18, screen, 6);
        draw_line(right_x - 18, eye_y - 18, right_x + 18, eye_y + 18, screen, 6);
        draw_line(right_x + 18, eye_y - 18, right_x - 18, eye_y + 18, screen, 6);
    }

    if (config.mouth_o) {
        draw_ellipse(160, 173, 20, 24, config.line_color);
        draw_ellipse(160, 173, 11, 14, screen);
    } else {
        draw_quadratic_curve(config.mouth[0], config.mouth[1],
                             config.mouth[2], config.mouth[3],
                             config.mouth[4], config.mouth[5],
                             config.line_color,
                             6,
                             30);
    }
    ...
    if (config.sweat) {
        draw_ellipse(251, 103, 6, 11, config.line_color);
    }
    if (config.anger) {
        draw_line(256, 54, 275, 41, config.line_color, 5);
        draw_line(266, 41, 266, 58, config.line_color, 4);
    }
}

void draw_robot_face_from_template(const char* emotion, int intensity_pct)
{
    draw_robot_face_config(robot_face_config(emotion, intensity_pct), true);
}

void draw_template_transient_frame(const char* base_emotion,
                                   int base_intensity,
                                   int eye_open_pct,
                                   int eye_size_delta_pct,
                                   int look_x,
                                   int look_y,
                                   int mouth_mode,
                                   int mouth_shift,
                                   ...
                                   int brow_phase = 0)
{
    RobotFaceConfig config = robot_face_config(base_emotion, base_intensity);
    if (eye_open_pct >= 0) {
        config.eye_open_pct = clamp_int(eye_open_pct, 8, 125);
    }
    config.eye_size_pct = clamp_int(config.eye_size_pct + eye_size_delta_pct, 90, 112);
    config.look_x = clamp_int(look_x, -16, 16);
    config.look_y = clamp_int(look_y, -12, 12);
    apply_robot_brow_pose(config, brow_mode, brow_phase);

    if (mouth_mode == 1) {
        copy_robot_mouth(config.mouth, 116, 166 + mouth_shift, 160, 191 + mouth_shift, 204, 166 + mouth_shift);
    } else if (mouth_mode == 2) {
        copy_robot_mouth(config.mouth, 136, 176 + mouth_shift, 160, 181 + mouth_shift, 184, 176 + mouth_shift);
    } else if (mouth_mode == 3) {
        copy_robot_mouth(config.mouth, 124 + mouth_shift, 176, 160 + mouth_shift, 181, 196 + mouth_shift, 176);
    }

    draw_robot_face_config(config, false);
}

bool animate_template_transient_face(const char* emotion, int intensity_pct)
{
    if (!is_transient_face_emotion(emotion)) {
        return false;
    }
    ...
    wake_display_if_needed();
    copy_ui_mode("face");
    g_info_mode_active = false;
    const char* base_emotion = normalize_face_emotion(g_face_emotion);
    const int base_intensity = clamp_int(g_face_intensity_pct > 0 ? g_face_intensity_pct : intensity_pct, 35, 90);

    if (str_eq(emotion, "blink") || str_eq(emotion, "soft_blink")) {
        const int heights[] = {100, 72, 30, 10, 36, 78, 100};
        for (int height : heights) {
            draw_template_transient_frame(base_emotion, base_intensity, height, 0, 0, 0, 0, 0);
            vTaskDelay(pdMS_TO_TICKS(32));
        }
        draw_robot_face_from_template(base_emotion, base_intensity);
        return true;
    }

    if (str_eq(emotion, "breathe") || str_eq(emotion, "deep_breathe")) {
        const bool deep = str_eq(emotion, "deep_breathe");
        const int phases[] = {0, 1, 3, 5, 7, 5, 3, 1, 0, -2, -1, 0};
        for (int phase : phases) {
            const int breath_open = clamp_int(96 + phase * (deep ? 4 : 3), 78, 122);
            const int breath_size = clamp_int(phase * (deep ? 2 : 1), -3, 14);
            const int breath_look_y = clamp_int(-phase / 2, -5, 3);
            const int mouth_mode = phase > 1 ? 1 : 2;
            draw_template_transient_frame(base_emotion,
                                          clamp_int(base_intensity + phase * 2, 35, 94),
                                          breath_open,
                                          breath_size,
                                          0,
                                          breath_look_y,
                                          ...
                                          phase,
                                          4,
                                          clamp_int(phase / 2, -2, 4));
            vTaskDelay(pdMS_TO_TICKS(deep ? 125 : 95));
        }
        draw_robot_face_from_template(base_emotion, base_intensity);
        return true;
    }

    if (str_eq(emotion, "brow_raise") ||
        str_eq(emotion, "brow_soft") ||
        str_eq(emotion, "brow_skeptic") ||
        str_eq(emotion, "brow_skeptic_left") ||
        str_eq(emotion, "brow_skeptic_right") ||
        str_eq(emotion, "brow_wiggle")) {
        int brow_mode = 1;
        if (str_eq(emotion, "brow_soft")) {
            brow_mode = 4;
        } else if (str_eq(emotion, "brow_skeptic") || str_eq(emotion, "brow_skeptic_left")) {
            brow_mode = 2;
        } else if (str_eq(emotion, "brow_skeptic_right")) {
            brow_mode = 3;
        } else if (str_eq(emotion, "brow_wiggle")) {
            brow_mode = 5;
        }
        const int smooth_phases[] = {0, 1, 2, 3, 2, 1, 0};
        const int wiggle_phases[] = {0, 1, -1, 2, -2, 1, 0};
        const int* phases = brow_mode == 5 ? wiggle_phases : smooth_phases;
        const int phase_count = brow_mode == 5 ? static_cast<int>(sizeof(wiggle_phases) / sizeof(wiggle_phases[0]))
                                                : static_cast<int>(sizeof(smooth_phases) / sizeof(smooth_phases[0]));
        for (int i = 0; i < phase_count; ++i) {
            draw_template_transient_frame(base_emotion,
                                          ...
                                          brow_mode,
                                          phases[i]);
            vTaskDelay(pdMS_TO_TICKS(brow_mode == 5 ? 70 : 90));
        }
        draw_robot_face_from_template(base_emotion, base_intensity);
        return true;
    }
    ...
    int target_x = 0;
    int target_y = 0;
    if (str_eq(emotion, "glance_left") || str_eq(emotion, "look_left")) {
        target_x = -12;
    } else if (str_eq(emotion, "glance_right") || str_eq(emotion, "look_right")) {
        target_x = 12;
    } else if (str_eq(emotion, "glance_up") || str_eq(emotion, "look_up")) {
        target_y = -9;
    } else if (str_eq(emotion, "glance_down") || str_eq(emotion, "look_down")) {
        target_y = 9;
    }
    if (target_x != 0 || target_y != 0) {
        const float phases[] = {0.0f, 0.55f, 1.0f, 1.0f, 0.55f, 0.0f};
        for (float phase : phases) {
            draw_template_transient_frame(base_emotion,
                                          base_intensity,
                                          -1,
                                          0,
                                          static_cast<int>(std::lround(target_x * phase)),
                                          static_cast<int>(std::lround(target_y * phase)),
                                          0,
                                          0,
                                          target_y < 0 ? 1 : target_x < 0 ? 2 : target_x > 0 ? 3 : target_y > 0 ? 4 : 0,
                                          static_cast<int>(std::lround(2.0f * phase)));
            vTaskDelay(pdMS_TO_TICKS(95));
        }
        draw_robot_face_from_template(base_emotion, base_intensity);
        return true;
    ...
    if (str_eq(emotion, "mouth_smile") || str_eq(emotion, "mouth_tiny") || str_eq(emotion, "mouth_wiggle")) {
        const int mode = str_eq(emotion, "mouth_smile") ? 1 : str_eq(emotion, "mouth_tiny") ? 2 : 3;
        const int shifts[] = {0, 2, -1, 1, 0};
        for (int shift : shifts) {
            draw_template_transient_frame(base_emotion, base_intensity, -1, 0, 0, 0, mode, shift);
            vTaskDelay(pdMS_TO_TICKS(90));
        }
        draw_robot_face_from_template(base_emotion, base_intensity);
        return true;
...
void draw_info_mini_face()
{
    ...
    draw_ellipse(ox + 30, oy + 30, 8, 13, line);
    draw_ellipse(ox + 66, oy + 30, 8, 13, line);
    draw_ellipse(ox + 30, oy + 31, 3, 5, cutout);
    draw_ellipse(ox + 66, oy + 31, 3, 5, cutout);
    draw_line(ox + 18, oy + 12, ox + 40, oy + 10, line, 2);
    draw_line(ox + 56, oy + 10, ox + 78, oy + 12, line, 2);
    draw_quadratic_curve(ox + 34, oy + 66, ox + 48, oy + 76, ox + 62, oy + 66, line, 3, 16);
}

void draw_info_screen(const char* time_text, const char* date_text, const char* weekday_text)
{
    char clean_time[sizeof(g_info_time)] = {};
    char clean_date[sizeof(g_info_date)] = {};
    char clean_weekday[sizeof(g_info_weekday)] = {};
    copy_display_text(clean_time, sizeof(clean_time), time_text && *time_text ? time_text : "--:--");
    copy_display_text(clean_date, sizeof(clean_date), date_text && *date_text ? date_text : "--.--.----");
    copy_display_text(clean_weekday, sizeof(clean_weekday), weekday_text && *weekday_text ? weekday_text : "INFO");
    copy_cstr(g_info_time, sizeof(g_info_time), clean_time);
    copy_cstr(g_info_date, sizeof(g_info_date), clean_date);
    copy_cstr(g_info_weekday, sizeof(g_info_weekday), clean_weekday);
    ...
    draw_centered_text(154, g_info_date, 3, line);
    draw_info_mini_face();
}

void draw_face(const char* emotion, int intensity_pct)
{
    if (!accepts_robot_template_face_command(emotion)) {
        ESP_LOGD(kTag, "ignored non-template face emotion: %s", emotion ? emotion : "");
        return;
    }
    ...
    const bool was_plain_face = std::strcmp(g_ui_mode, "face") == 0 && !g_info_mode_active;
    if (!g_force_face_redraw &&
        !g_motion_gaze_active &&
        g_motion_gaze_render_dx == 0 &&
        g_motion_gaze_render_dy == 0 &&
        std::strcmp(g_ui_mode, "face") == 0 &&
        str_eq(g_face_emotion, target_emotion) &&
        std::abs(g_face_intensity_pct - target_intensity) < 6) {
        return;
    }

    if (str_eq(target_emotion, "listening")) {
        copy_ui_mode("recording");
    } else if (str_eq(target_emotion, "speaking")) {
        copy_ui_mode("speaking");
    } else {
        copy_ui_mode("face");
    }
    g_info_mode_active = false;
    g_face_partial_update_next = was_plain_face && std::strcmp(g_ui_mode, "face") == 0;
    const bool should_transition =
        !g_force_face_redraw &&
        !g_motion_gaze_active &&
        std::strcmp(g_ui_mode, "face") == 0 &&
        !str_eq(g_face_emotion, target_emotion);
    const bool animated = should_transition && animate_face_transition(target_emotion, target_intensity);
    copy_face_emotion(target_emotion, target_intensity);
    if (!animated) {
        FacePose pose = {};
        if (build_face_pose(g_face_emotion, g_face_intensity_pct, pose)) {
            render_face_pose(pose);
        } else {
            draw_robot_face_from_template(g_face_emotion, g_face_intensity_pct);
        }
    }
    g_face_partial_update_next = false;
}
...
{
    ESP_LOGE(kTag, "display error: %s", message ? message : "");
    draw_face("scared", 75);
}
...
void set_motion_gaze_from_pct_delta(int yaw_delta_pct, int pitch_delta_pct, bool draw_now)
{
    ...
    if (std::abs(yaw_delta_pct) > kDeadbandPct) {
        // Yaw servo sign is device-centric; the face must read correctly
        // from the human looking at the display.
    ...
    if (draw_now &&
        ...
        std::strcmp(g_ui_mode, "display") != 0 &&
        std::strcmp(g_ui_mode, "image") != 0) {
        g_motion_gaze_render_dx = dx;
        g_motion_gaze_render_dy = dy;
        draw_motion_gaze_partial(dx, dy);
    }
}
...
int smooth_gaze_value(int current, int target)
{
    const int diff = target - current;
    if (diff == 0) {
        return current;
    }
    int step = std::max(1, static_cast<int>(std::lround(std::abs(diff) * 0.30f)));
    step = std::min(step, g_head_motion_active ? 3 : 6);
...
void motion_gaze_task(void*)
{
    ...
    while (true) {
        ...
        const int target_dx = active ? clamp_int(static_cast<int>(g_motion_gaze_dx), -18, 18) : 0;
        const int target_dy = active ? clamp_int(static_cast<int>(g_motion_gaze_dy), -14, 14) : 0;
        const int current_dx = clamp_int(static_cast<int>(g_motion_gaze_render_dx), -18, 18);
        const int current_dy = clamp_int(static_cast<int>(g_motion_gaze_render_dy), -14, 14);
        const int next_dx = smooth_gaze_value(current_dx, target_dx);
        const int next_dy = smooth_gaze_value(current_dy, target_dy);

        if (can_draw) {
            const bool changed_target = revision != last_revision;
            const bool changed_frame = next_dx != current_dx || next_dy != current_dy;
            const bool needs_first_draw = active && !drew_visible_gaze;
            const int min_interval_ms = g_head_motion_active ? 120 : 70;
            if ((changed_target || changed_frame || needs_first_draw) &&
                (changed_target || needs_first_draw || now_ms - last_draw_ms >= min_interval_ms)) {
                g_motion_gaze_render_dx = next_dx;
                g_motion_gaze_render_dy = next_dy;
                last_revision = revision;
                last_draw_ms = now_ms;
                if (!draw_motion_gaze_partial(next_dx, next_dy)) {
                    render_current_face_pose_no_transition();
                }
                drew_visible_gaze = active || next_dx != 0 || next_dy != 0;
            }
        } else if (!active && (current_dx != 0 || current_dy != 0)) {
            g_motion_gaze_render_dx = 0;
            g_motion_gaze_render_dy = 0;
            drew_visible_gaze = false;
        }
        vTaskDelay(pdMS_TO_TICKS(g_head_motion_active ? 70 : 40));
    }
}
...
void publish_status()
{
    ...
    std::snprintf(payload,
                  ...
                  static_cast<int>(g_tts_playback_level_pct),
                  (g_head_touch_ready || g_display_touch_ready) ? "true" : "false",
                  g_head_touch_ready ? "true" : "false",
                  g_display_touch_ready ? "true" : "false",
...
{
    (void)yaw_change_pct;
    (void)pitch_change_pct;
    // The servo task drives motion gaze directly. Queueing a full face command
    // here stacks a second animation onto head moves and makes pitch commands hop.
}
...
void handle_display_command(const char* data, int len)
{
    ...
    if (std::strcmp(mode, "face") == 0) {
        ...
        copy_cstr(command.emotion, sizeof(command.emotion), g_face_emotion);
        if (!enqueue_ui_command(command)) {
            publish_error(request_id, "display", "ui queue full");
            cJSON_Delete(root);
            return;
        ...
        publish_ack(request_id, "display", "face queued");
        cJSON_Delete(root);
        return;
    }

    if (std::strcmp(mode, "info") == 0) {
        const char* time_text = json_string(root, "time");
        const char* date_text = json_string(root, "date");
        const char* weekday_text = json_string(root, "weekday");
        if (!time_text || !*time_text || !date_text || !*date_text || !weekday_text || !*weekday_text) {
            publish_error(request_id, "display", "expected info time/date/weekday");
            cJSON_Delete(root);
            return;
        }

        UiCommand command = {};
        command.type = UiCommandType::Info;
        command.duration_ms = std::max(0, json_int(root, "duration_ms", 0));
        copy_display_text(command.info_time, sizeof(command.info_time), time_text);
        copy_display_text(command.info_date, sizeof(command.info_date), date_text);
        copy_display_text(command.info_weekday, sizeof(command.info_weekday), weekday_text);
        ESP_LOGI(kTag, "info display command queued: %s %s %s request_id=%s",
                 command.info_time,
                 command.info_weekday,
                 command.info_date,
                 request_id && *request_id ? request_id : "");
        if (!enqueue_ui_command(command)) {
            publish_error(request_id, "display", "ui queue full");
        ...
        publish_ack(request_id, "display", "info queued");
        cJSON_Delete(root);
        return;
    }
    ...
    if (std::strcmp(mode, "text") != 0 || !text || !*text) {
        publish_error(request_id, "display", "expected mode text/image/info/face and valid payload");
        cJSON_Delete(root);
        return;
    }
...
void handle_face_command(const char* data, int len)
{
    ...
    if (g_info_mode_active && request_id && std::strncmp(request_id, "life-", 5) == 0) {
        publish_ack(request_id, "face", "life face ignored during info mode");
        cJSON_Delete(root);
        return;
    }

    if (!accepts_robot_template_face_command(emotion)) {
        ESP_LOGD(kTag, "ignored non-template face command: %s", emotion ? emotion : "");
        publish_ack(request_id, "face", "non-template face ignored");
        cJSON_Delete(root);
        return;
    }
...
void set_recording_state(bool enabled, const char* source, const char* request_id, const char* reason)
{
    if (enabled) {
        ...
        g_pre_recording_info_mode = g_info_mode_active && std::strcmp(g_ui_mode, "info") == 0;
        ...
        if (g_pre_recording_info_mode && g_info_time[0] != '\0') {
            draw_info_screen(g_info_time, g_info_date, g_info_weekday);
        } else {
        ...
        g_pre_recording_info_mode = false;
...
void handle_audio_command(const char* data, int len)
{
    ...
    if (std::strcmp(action, "set_wakeword") == 0) {
        ...
        publish_status();
    } else if (std::strcmp(action, "stop_playback") == 0 || std::strcmp(action, "stop") == 0) {
        g_tts_stop_requested = true;
        publish_ack(request_id, "audio", "tts stop requested");
        publish_status();
    } else {
...
int pcm16_level_pct(const uint8_t* data, int len)
{
    if (!data || len < 2) {
        return 0;
    }
    const int sample_count = (len & ~1) / 2;
    if (sample_count <= 0) {
        return 0;
    }
    int64_t sum_abs = 0;
    ...
    const int avg = static_cast<int>(sum_abs / sample_count);
    const int avg_pct = (avg * 100) / 4500;
    const int peak_pct = (peak * 100) / 17000;
    return clamp_int(std::max(avg_pct, peak_pct), 0, 100);
}

void update_tts_playback_level(WavPlaybackState& state, const uint8_t* data, int len)
{
    (void)state;
    if (!g_tts_playing || !data || len < 2) {
        return;
    }
    const int raw_level = pcm16_level_pct(data, len);
    const int previous = clamp_int(static_cast<int>(g_tts_playback_level_pct), 0, 100);
    const int smoothed = clamp_int((previous * 58 + raw_level * 42) / 100, 0, 100);
    if (std::abs(smoothed - previous) >= 2) {
        g_tts_playback_level_pct = smoothed;
        g_tts_playback_level_revision = g_tts_playback_level_revision + 1;
    }
}

void tts_face_task(void*)
{
    uint32_t last_revision = g_tts_playback_level_revision;
    int last_level = -1;
    int64_t last_draw_ms = 0;
    while (true) {
        const bool active =
            g_tts_playing &&
            !g_display_sleeping &&
            std::strcmp(g_ui_mode, "speaking") == 0;

        if (!active) {
            last_revision = g_tts_playback_level_revision;
            last_level = -1;
            vTaskDelay(pdMS_TO_TICKS(80));
            continue;
        }

        const uint32_t revision = g_tts_playback_level_revision;
        const int level = clamp_int(static_cast<int>(g_tts_playback_level_pct), 0, 100);
        const int64_t now_ms = esp_timer_get_time() / 1000;
        const bool draw_due = now_ms - last_draw_ms >= 80;
        const bool level_changed = revision != last_revision || std::abs(level - last_level) >= 3;

        if (draw_due && (level_changed || level > 3)) {
            last_revision = revision;
            last_level = level;
            last_draw_ms = now_ms;
            if (!draw_speaking_mouth_partial(level)) {
                vTaskDelay(pdMS_TO_TICKS(35));
            }
        }
...
void write_aligned_pcm16(WavPlaybackState& state, const uint8_t* data, int len)
{
    ...
    if (state.has_pending_byte) {
        ...
        update_tts_playback_level(state, sample, sizeof(sample));
    ...
    if (aligned_len > 0) {
        update_tts_playback_level(state, data, aligned_len);
...
bool play_wav_url(const char* url)
{
    ...
    g_tts_playback_level_pct = 0;
    g_tts_playback_level_revision = g_tts_playback_level_revision + 1;
    g_pre_tts_info_mode = g_info_mode_active && std::strcmp(g_ui_mode, "info") == 0;
    copy_cstr(g_pre_tts_face_emotion,
              sizeof(g_pre_tts_face_emotion),
              g_face_emotion[0] ? g_face_emotion : "playful");
    g_pre_tts_face_intensity_pct = g_face_intensity_pct;
    draw_face("speaking", 72);
    publish_status();
    const esp_err_t err = esp_http_client_perform(client);
    const int status = esp_http_client_get_status_code(client);
    esp_http_client_cleanup(client);
    ...
    g_tts_playback_level_pct = 0;
    g_tts_playback_level_revision = g_tts_playback_level_revision + 1;
    if (g_pre_tts_info_mode && g_info_time[0] != '\0') {
        draw_info_screen(g_info_time, g_info_date, g_info_weekday);
    } else {
        draw_face(g_pre_tts_face_emotion, g_pre_tts_face_intensity_pct);
    }
    g_pre_tts_info_mode = false;
    publish_status();
...
void ui_task(void*)
{
    ...
    while (true) {
        ...
        if (command.type == UiCommandType::Display) {
            ...
            draw_face(g_face_emotion, g_face_intensity_pct);
            publish_status();
        } else if (command.type == UiCommandType::Info) {
            draw_info_screen(command.info_time, command.info_date, command.info_weekday);
            publish_status();
            ...
            if (accepts_robot_template_face_command(command.emotion)) {
...
void touch_event_task(void*)
{
    ...
    int64_t active_touch_started_ms = 0;
    ...
    while (true) {
        ...
        if (stable_count >= 2 && pressed != stable_pressed) {
            ...
            if (pressed) {
                ...
                active_touch_started_ms = now_ms;
            }
            ...
            if (pressed && touch_starts_recording && head_pressed) {
                if (g_audio_input_ready && g_audio_input) {
                    set_recording_state(true,
                                        source,
                                        "",
                                        "touch pressed");
                    active_touch_started_recording = true;
                } else {
                    draw_face("error", 80);
            ...
            if (!pressed) {
                const int64_t touch_duration_ms = active_touch_started_ms > 0 ? now_ms - active_touch_started_ms : 0;
                if (std::strcmp(active_source, "display_touch") == 0 &&
                    !active_touch_started_recording &&
                    touch_duration_ms <= kDisplayTouchTapMaxMs) {
                    publish_touch_event("touch_tap",
                                        "display_touch",
                                        "display",
                                        raw,
                                        0,
        ...
        if (stable_pressed &&
            !active_touch_started_recording &&
            std::strcmp(active_source, "display_touch") == 0 &&
            now_ms - active_touch_started_ms >= kDisplayTouchHoldToRecordMs) {
            if (g_audio_input_ready && g_audio_input) {
                set_recording_state(true,
                                    "display_touch",
                                    "",
                ...
                draw_face("error", 80);
                publish_event("recording_error", "display_touch", "", "microphone not ready");
                active_touch_started_recording = true;
            }
        }
...
extern "C" void app_main()
{
    ...
    xTaskCreatePinnedToCore(tts_face_task, "tts_face", 4096, nullptr, 1, nullptr, kTtsFaceCore);
